...
def swap_pairpot_splines(ff, rmin=1.0*angstrom, rmax=None, npoints=1000):
    '''
    ...
    for part in ff.parts:
        if isinstance(part, ForcePartPair):
            ...
            newff.add_part(part)
    pairpotff.compute()
    pairpotspline = ForcePartSpline(pairpotff, rmin=rmin, rmax=rmax, npoints=npoints)
...
class ForcePartSpline(ForcePart):
    ...
    '''
    def __init__(self, ncff, rmin=1.0*angstrom, rmax=None, npoints=1000):
        ForcePart.__init__(self, 'splines', ncff.system)
        ...
        if rmax is None: rmax = self.nlist.rcut
